...
//        cout << endl;
        for (uint32_t i = 0; i < uPages; i++) {
            ...
            //strSHASum 转 base64 输出测试
//            cout << DataBase64(strSHASum) << endl;
            strOutput.append(strSHASum.data(), strSHASum.size());
        }
        auto s = strOutput.size();
        if (uRemain > 0) {
            string strSHASum;
...
bool SlotBuildCMSSignature(ZSignAsset *pSignAsset,
    ...
    auto strCodeDirectorySlotLength = strCodeDirectorySlot.size();
    ...
    auto ss3 = strCodeDirectorySlotSHA1.size();
    auto ss1 = DataBase64(strCodeDirectorySlotSHA1.data());
    auto ss2 = DataBase64(strAltnateCodeDirectorySlot256.data());
    ...
    auto aa = DataBase64(strCDHashesPlist);
    auto s1 = strCodeDirectorySlot.size();
    auto s2 = strCDHashesPlist.size();
    auto s = strCMSData.size();
